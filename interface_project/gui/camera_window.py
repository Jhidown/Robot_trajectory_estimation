...
def open_camera_window(root,ekf):
    ...
    def update_buttons(event=None):
        for widget in camera_button_frame.grid_slaves():
        ...
        if mode in ["VO", "VO + EKF", "VO + Encoders", "VO + EKF + Encoders"]:
            tk.Button(camera_button_frame, text="VO", width=15, command=camera_save_data).grid(row=1, column=1, padx=5, pady=5)
        if mode in ["EKF", "VO + EKF", "EKF + Encoders", "VO + EKF + Encoders"]:
            tk.Button(camera_button_frame, text='EKF', width=15, command=save_ekf_csv).grid(row=1, column=2, padx=5, pady=5)
        if mode in ["Encoders", "VO + Encoders", "EKF + Encoders", "VO + EKF + Encoders"]:
            tk.Button(camera_button_frame, text='Encoders', width=15, command=save_wheel_csv).grid(row=1, column=3, padx=5, pady=5)
    ...
    tk.Button(camera_button_frame, text="Stop Camera", width=15, fg="red", command=camera_stop).grid(row=0, column=3, padx=5, pady=5)
