...
from communication.serial_comm import init_motor, send_message_confirm
...
from logic.encoders_utils import process_encoder, check_distance_or_time
...
def run_gui():
    ...
    def insert_custom_message(message: str, tag: str):
        ...
        nonlocal last_valid_position, encoder_trajectory, encoder_pose, last_update_time
        ...
        # ===== Vérifier si message encodeur =====
        match_enc = re.search(r"\[ENC\]\s+ID=(\w+)\s+DATA=\[([0-9A-Fa-f\s]+)\]", message)
        if not match_enc:
            return
        else:
            can_id = int(match_enc.group(1), 16)
            data_str = match_enc.group(2).strip().split()
            data_bytes = bytes(int(b, 16) for b in data_str)

            result = process_encoder(can_id, data_bytes)
            if result is None:
                return
            else:
                delta_d, delta_theta = result
                ...
                # ===== Déclenchement caméra quand distance/temps atteints =====
                if check_distance_or_time(delta_d_total, threshold_dist=0.3, threshold_time=9.0):
                    timestamp = datetime.now().strftime("%H:%M:%S")
                    full_msg = f"[{timestamp}] Triggering camera capture"
                    text_history.insert(tk.END, full_msg + "\n", tag)
                    text_history.see(tk.END)
                    ...
                    if x_cam is not None and y_cam is not None:
                        ...
                        if new_position != last_valid_position:
                            ...
                            # EKF prédiction avec accumulation
                            if dt_accumulated > 0:
                                ekf.predict(delta_d_total, delta_theta_total, dt_accumulated)

                                # Mettre à jour trajectoire ENCODERS en même temps que VO
    ...
    def forward():
        send_message_confirm(0x601, [0x23, 0xFF, 0x60, 0x00, 0x68, 0xC5, 0xFF, 0xFF])
        send_message_confirm(0x602, [0x23, 0xFF, 0x60, 0x00, 0x98, 0x3A, 0x00, 0x00])


    def backward():
        send_message_confirm(0x601, [0x23, 0xFF, 0x60, 0x00, 0x98, 0x3A, 0x00, 0x00])
        send_message_confirm(0x602, [0x23, 0xFF, 0x60, 0x00, 0x68, 0xC5, 0xFF, 0xFF])


    def stop():
        send_message_confirm(0x601, [0x23, 0xFF, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00])
        send_message_confirm(0x602, [0x23, 0xFF, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00])


    def right():
        send_message_confirm(0x601, [0x23, 0xFF, 0x60, 0x00, 0x4C, 0x1D, 0x00, 0x00])
        send_message_confirm(0x602, [0x23, 0xFF, 0x60, 0x00, 0x4C, 0x1D, 0x00, 0x00])


    def left():
        send_message_confirm(0x601, [0x23, 0xFF, 0x60, 0x00, 0xB4, 0xE2, 0xFF, 0xFF])
        send_message_confirm(0x602, [0x23, 0xFF, 0x60, 0x00, 0xB4, 0xE2, 0xFF, 0xFF])
    ...
    open_camera_button = tk.Button(top_right_button_frame,text='Open camera',width=15,command=lambda: open_camera_window(root,ekf,vo))
